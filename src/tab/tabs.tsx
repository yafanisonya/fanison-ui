  ...
  setup(props, { slots }) {
    ...
    return () => (
      <div class="s-tabs">
        ...
        <ul class="s-tabs__nav">
          {tabsData.value.map(tab => (
            <li
              ...
              class={[
                tab.name === activeTab.value ? 'active' : '',
                tabsData.value.length === 1 ? 'only' : ''
              ]}
            >
              {tab.title}
              {/* 关闭页签 */}
              {(props.closable || props.editable) &&
        ...
        {/* 显示新增的内容 */}
        {activeTab.value.split('0')[0] === 'add' && (
          <div class="s-tabs__content">New Tab content</div>
        )}
      </div>
    )
  }
